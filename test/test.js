...
contract('DStorage', ([deployer, uploader]) => {
  ...
  describe('deployment', async () => {
    ...
    it('has a name', async () => {
      ...
      assert.equal(name, 'DApp dropbox')
    })
  })
  ...
  describe('file', async () => {
    ...
    const fileDesc = 'DescriptionOfTheFile'

    before(async () => {
      result = await dstorage.uploadFile(fileHash, fileSize, fileType, fileName, fileDesc, { from: uploader })
    ...
    it('upload file', async () => {
      ...
      assert.equal(event.fileDesc, fileDesc, 'Description is correct')
      assert.equal(event.uploader, uploader, 'Uploader is correct')

      // FAILURE: File must have hash
      await dstorage.uploadFile('', fileSize, fileType, fileName, fileDesc, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await dstorage.uploadFile(fileHash, '', fileType, fileName, fileDesc, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await dstorage.uploadFile(fileHash, fileSize, '', fileName, fileDesc, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await dstorage.uploadFile(fileHash, fileSize, fileType, '', fileDesc, { from: uploader }).should.be.rejected;
    ...
    it('lists file', async () => {
      ...
      assert.equal(file.fileDesc, fileDesc, 'description is correct')
